...
```{r,echo=FALSE}
make_bubble_plot <- function(diff_pvalue_file, pip_cutoff, pvalue_cutoff, title){
  gene_pip <- read.table('./data/large_pipsumgenes.tsv',sep='\t',header=TRUE)
  gene_pip <- gene_pip[c('Symbol','pip_tot')]
  colnames(gene_pip) = c('gene','pip')
  ...
  ggplot(pip_p, aes(x=pip,y=gene,size=padj))+geom_point(alpha=0.7)+labs(title = diff_pvalue_file)+theme(axis.text=element_text(size=12),axis.title=element_text(size=14,hjust = 0.5))
}
```
...
library(grid)
library(gridExtra)
p1 <- make_bubble_plot('dec.PLXdec.TL',pip_cutoff,pvalue_cutoff, "dec.PL_dec.TL")
p2 <- make_bubble_plot('dec.PLXtcm.PL',pip_cutoff,pvalue_cutoff, "dec.PL_tcm.PL")
p3 <- make_bubble_plot('dec.TLXtcm.TL',pip_cutoff,pvalue_cutoff, "dec.TL_tcm.TL")
p4 <- make_bubble_plot('tcm.PLXtcm.TL',pip_cutoff,pvalue_cutoff, "tcm.PL_tcm.TL")
p5 <- make_bubble_plot('unt.PLXctr.TL',pip_cutoff,pvalue_cutoff, "unt.PL_ctr.TL")
p6 <- make_bubble_plot('unt.PLXdec.PL',pip_cutoff,pvalue_cutoff, "unt.PL_dec.PL")
p7 <- make_bubble_plot('unt.TLXdec.TL',pip_cutoff,pvalue_cutoff, "unt.TL_dec.TL")
