...
layout(mat = matrix(c(1, 2), nrow = 1),widths = c(1, 0.2))
p1 <- plot(as.matrix(df_heatmap),axis.col=list(las=2, cex.axis=1), axis.row=list(las=2, cex.axis=1),xlab="", ylab="", main="-log10(p-value)",col=brewer.pal(8,name = "Reds"),spacing.key=c(1,0.5,0))
p2 <- plot(as.matrix(df1$pip),digits=4,key=NULL,col='white',axis.col=NULL, axis.row=NULL,xlab="", ylab="",main="PIP")
```
